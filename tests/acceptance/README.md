...
Run the installed skill through an agent against the expected authenticated Fulcra account. Store raw results, acceptance receipts, and cleanup manifests under `.acceptance-runs/<skill-name>/<run-id>/`, outside this tree. These artifacts remain local and are never committed or published. Follow `docs/agents/skill-authoring.md` for completion criteria and live-account handling.
