...
This obligation follows from ADR 0001: publishing the CLI implicitly updates every distributed skill even when no skill file changes. Release acceptance must therefore cover the skills as downstream consumers, verify real agent behavior rather than only CLI availability, preserve sanitized receipts, and surface incomplete cleanup.
