...
When a previously projected Computer History Memory remains present but its content hash changes, the collector treats the change as a revision. It uploads the file to the same remote path so Fulcra creates a new file snapshot, then advances the corresponding Timeline record to the revised Markdown instead of leaving overlapping records for the same memory.
